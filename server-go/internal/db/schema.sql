...
-- Planned services / mods (shopping + cost planning)
CREATE TABLE IF NOT EXISTS plans (
    id TEXT PRIMARY KEY,
    vehicle_id TEXT NOT NULL,
    user_id TEXT,
    plan_kind TEXT NOT NULL, -- service | upgrade
    title TEXT NOT NULL,
    service_type TEXT,
    category TEXT,
    notes TEXT,
...
CREATE INDEX IF NOT EXISTS idx_plans_vehicle ON plans (vehicle_id, status, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS plan_parts (
    ...
    quantity REAL NOT NULL DEFAULT 1,
    unit_cost REAL NOT NULL DEFAULT 0,
    acquisition TEXT NOT NULL DEFAULT 'need', -- need (not bought) | owned (from parts inventory)
    notes TEXT,
    created_at TEXT NOT NULL DEFAULT (datetime('now')),
