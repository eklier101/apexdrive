...
func (a *API) listServices(w http.ResponseWriter,r *http.Request){id,ok:=a.vehicleQuery(w,r);if !ok{return};rows,err:=a.db.Query(`SELECT * FROM services WHERE vehicle_id=? ORDER BY date DESC,odometer DESC`,id);if err==nil{rows,err=attachParts(a.db,rows)};if err!=nil{writeError(w,500,err);return};writeJSON(w,200,rows)}
...
func (a *API) createService(w http.ResponseWriter,r *http.Request){
	b,err:=bodyMap(r);if err!=nil{writeError(w,400,"Invalid JSON");return};if text(b["vehicle_id"])==""||text(b["service_type"])==""||text(b["title"])==""{writeError(w,400,"vehicle_id, odometer, service_type, and title are required");return};if _,ok:=b["odometer"];!ok{writeError(w,400,"vehicle_id, odometer, service_type, and title are required");return};if _,ok:=a.ownedVehicle(w,r,text(b["vehicle_id"]));!ok{return}
...
func (a *API) updateService(w http.ResponseWriter,r *http.Request){
	id:=chi.URLParam(r,"id");old,err:=a.db.QueryOne(`SELECT * FROM services WHERE id=?`,id);if err!=nil{writeError(w,500,err);return};if old==nil{writeError(w,404,"Service record not found");return};if _,ok:=a.ownedVehicle(w,r,text(old["vehicle_id"]));!ok{return};b,err:=bodyMap(r);if err!=nil{writeError(w,400,"Invalid JSON");return}
...
func (a *API) deleteService(w http.ResponseWriter,r *http.Request){id:=chi.URLParam(r,"id");old,err:=a.db.QueryOne(`SELECT * FROM services WHERE id=?`,id);if err!=nil{writeError(w,500,err);return};if old==nil{writeError(w,404,"Service record not found");return};if _,ok:=a.ownedVehicle(w,r,text(old["vehicle_id"]));!ok{return};if err=svc.RestoreServiceParts(a.db,id);err==nil{_,err=a.db.Exec(`DELETE FROM services WHERE id=?`,id)};if err==nil{err=svc.SyncVehicleReminders(a.db,text(old["vehicle_id"]))};if err!=nil{writeError(w,500,err);return};writeJSON(w,200,map[string]any{"message":"Service deleted successfully"})}
