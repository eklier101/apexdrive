...
func(a *API)dashboard(w http.ResponseWriter,r *http.Request){id,ok:=vehicleQuery(w,r);if !ok{return};if err:=svc.SyncVehicleReminders(a.db,id);err!=nil{writeError(w,500,err);return};stats,err:=svc.GetDashboardStats(a.db,id);if err!=nil{writeError(w,500,err);return};if stats==nil{writeError(w,404,"Vehicle not found");return};writeJSON(w,200,stats)}
