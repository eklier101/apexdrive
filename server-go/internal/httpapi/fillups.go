...
func vehicleQuery(w http.ResponseWriter,r *http.Request)(string,bool){id:=r.URL.Query().Get("vehicle_id");if id==""{writeError(w,400,"vehicle_id query parameter is required");return "",false};return id,true}
func (a *API) listFillups(w http.ResponseWriter,r *http.Request){id,ok:=vehicleQuery(w,r);if !ok{return};rows,err:=a.db.Query(`SELECT * FROM fillups WHERE vehicle_id=? ORDER BY odometer DESC,date DESC`,id);if err!=nil{writeError(w,500,err);return};writeJSON(w,200,rows)}
func (a *API) latestFillup(w http.ResponseWriter,r *http.Request){id,ok:=vehicleQuery(w,r);if !ok{return};row,err:=a.db.QueryOne(`SELECT * FROM fillups WHERE vehicle_id=? ORDER BY odometer DESC,date DESC LIMIT 1`,id);if err!=nil{writeError(w,500,err);return};writeJSON(w,200,row)}
...
func (a *API) deleteFillup(w http.ResponseWriter,r *http.Request){id:=chi.URLParam(r,"id");old,err:=a.db.QueryOne(`SELECT * FROM fillups WHERE id=?`,id);if err!=nil{writeError(w,500,err);return};if old==nil{writeError(w,404,"Fillup not found");return};if _,err=a.db.Exec(`DELETE FROM fillups WHERE id=?`,id);err!=nil{writeError(w,500,err);return};if err=services.RecalculateVehicleFillups(a.db,text(old["vehicle_id"]));err!=nil{writeError(w,500,err);return};writeJSON(w,200,map[string]any{"message":"Fillup deleted successfully"})}
