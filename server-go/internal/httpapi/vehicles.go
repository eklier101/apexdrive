...
func (a *API) vehicleRoutes(r chi.Router) {
	r.Route("/vehicles", func(r chi.Router) {
		r.Get("/", a.listVehicles); r.Post("/", a.createVehicle)
		r.Get("/{id}", a.getVehicle); r.Put("/{id}", a.updateVehicle); r.Delete("/{id}", a.deleteVehicle)
	})
}

func (a *API) listVehicles(w http.ResponseWriter, r *http.Request) {
	var rows []map[string]any; var err error
	...
	} else { rows, err = a.db.Query(`SELECT * FROM vehicles ORDER BY is_active DESC,created_at ASC`) }
	if err != nil { writeError(w,500,err); return }; writeJSON(w,200,rows)
}

func (a *API) getVehicle(w http.ResponseWriter, r *http.Request) {
	row, err := a.db.QueryOne(`SELECT * FROM vehicles WHERE id=?`, chi.URLParam(r,"id"))
	if err != nil { writeError(w,500,err); return }; if row == nil { writeError(w,404,"Vehicle not found"); return }; writeJSON(w,200,row)
}
...
func (a *API) createVehicle(w http.ResponseWriter, r *http.Request) {
	...
	if text(b["name"])=="" || text(b["make"])=="" || text(b["model"])=="" || num(b["year"])==0 { writeError(w,400,"Name, make, model, and year are required"); return }
	id, userID := db.NewID("veh"), any(nil); if u:=userFromContext(r); u!=nil { userID=u.UserID }
	...
	if err != nil { writeError(w,500,err); return }; row,_:=a.db.QueryOne(`SELECT * FROM vehicles WHERE id=?`,id); writeJSON(w,201,row)
}

func (a *API) updateVehicle(w http.ResponseWriter, r *http.Request) {
	id:=chi.URLParam(r,"id"); old,err:=a.db.QueryOne(`SELECT * FROM vehicles WHERE id=?`,id); if err!=nil{writeError(w,500,err);return}; if old==nil{writeError(w,404,"Vehicle not found");return}
	b,err:=bodyMap(r);if err!=nil{writeError(w,400,"Invalid JSON");return}
	...
	for _,k:=range keys { if v,ok:=b[k];ok { switch k {case "year","is_active": vals=append(vals,integer(v)); case "tank_capacity","purchase_price","purchase_odometer": vals=append(vals,num(v)); default: vals=append(vals,v)} } else { vals=append(vals,old[k]) } }
	vals=append(vals,id)
	...
	if err!=nil{writeError(w,500,err);return}; row,_:=a.db.QueryOne(`SELECT * FROM vehicles WHERE id=?`,id);writeJSON(w,200,row)
}

func (a *API) deleteVehicle(w http.ResponseWriter,r *http.Request){
	id:=chi.URLParam(r,"id");row,err:=a.db.QueryOne(`SELECT id FROM vehicles WHERE id=?`,id);if err!=nil{writeError(w,500,err);return};if row==nil{writeError(w,404,"Vehicle not found");return}
	if _,err=a.db.Exec(`DELETE FROM vehicles WHERE id=?`,id);err!=nil{writeError(w,500,err);return};writeJSON(w,200,map[string]any{"success":true,"message":"Vehicle deleted successfully"})
}
