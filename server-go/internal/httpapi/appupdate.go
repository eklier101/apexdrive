...
func (a *API) appVersion(w http.ResponseWriter, r *http.Request) {
	...
	if row == nil {
		writeJSON(w, 200, map[string]any{"available": false, "apk_valid": false, "version": "1.0.0", "version_code": 1, "message": "No APK release uploaded yet"})
		return
	}
	path := filepath.Join(a.cfg.APKsDir, filepath.Base(text(row["apk_filename"])))
	validErr := apkembed.ValidateAPKFile(path)
	out := map[string]any{"available": validErr == nil, "download_url": "/api/app/download-latest", "apk_valid": validErr == nil}
	for _, k := range []string{"id", "version", "version_code", "release_notes", "apk_size", "release_date"} {
		out[k] = row[k]
	}
	if validErr != nil {
		out["available"] = false
		out["apk_error"] = validErr.Error()
		out["message"] = "APK on server failed integrity check and will not be offered for download"
	}
	writeJSON(w, 200, out)
}
...
func (a *API) sendAPK(w http.ResponseWriter, r *http.Request, row map[string]any) {
	...
	if err := apkembed.ValidateAPKFile(path); err != nil {
		writeError(w, 422, fmt.Sprintf("APK failed integrity check (not installable): %v", err))
		return
	}
...
func (a *API) uploadRelease(w http.ResponseWriter, r *http.Request) {
	...
	if err := apkembed.ValidateAPKFile(path); err != nil {
		os.Remove(path)
		writeError(w, 400, fmt.Sprintf("Uploaded file is not a valid signed APK: %v", err))
		return
	}
