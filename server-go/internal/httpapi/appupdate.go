...
func (a *API) sendAPK(w http.ResponseWriter, r *http.Request, row map[string]any) {
	...
	// Embed download-origin server URL into APK when signing is configured.
	// Native in-app updates pass preserve_signature=1 so the package signature stays
	// identical to what is already installed (URL is already saved on the device).
	preserve := r.URL.Query().Get("preserve_signature") == "1" ||
		strings.EqualFold(r.Header.Get("X-ApexDrive-Client"), "native")
	if origin != "" && !preserve {
		if patched, err := apkembed.InjectServerURL(path, origin); err == nil && len(patched) > 0 {
			if signed, err := apkembed.TrySignAPK(patched); err == nil && len(signed) > 0 {
				w.Header().Set("Content-Length", strconv.Itoa(len(signed)))
				w.WriteHeader(200)
				_, _ = w.Write(signed)
				return
			}
			// Debug only: unsigned embed (install will fail on most devices)
			if os.Getenv("APK_ALLOW_UNSIGNED_EMBED") == "1" {
