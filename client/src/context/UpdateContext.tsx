...
export const UpdateProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const checkForUpdates = useCallback(async (manual: boolean = false) => {
    // Play Store builds update via Play — do not offer server APK installs.
    if (Capacitor.isNativePlatform() && !allowsSideloadUpdates()) {
      setHasUpdate(false);
      setLatestRelease(null);
      if (manual) {
        alert(`You are running v${CURRENT_APP_VERSION}. Updates come from the Google Play Store.`);
      }
      return;
    }
    ...
    try {
      ...
      if (releaseInfo && releaseInfo.available && releaseInfo.version_code > CURRENT_VERSION_CODE) {
        ...
        // Auto download on Wi-Fi if enabled (ONLY on native Android sideload app)
