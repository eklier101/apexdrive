...
import { Upgrade, Plan } from '../../types';
import { api } from '../../api/client';
import { asNumber } from '../../utils/numbers';
import { planToUpgradeDefaults } from '../../utils/planApply';

interface UpgradeModalProps {
  isOpen: boolean;
  onClose: () => void;
  upgradeToEdit?: Upgrade | null;
  defaults?: Partial<Upgrade> | null;
...
export const UpgradeModal: React.FC<UpgradeModalProps> = ({
  ...
  const isEditing = Boolean(upgradeToEdit?.id);
  const seed = isEditing ? upgradeToEdit : defaults;
  ...
  const [category, setCategory] = useState<string>(seed?.category || 'Performance');
  const [brandPartNumber, setBrandPartNumber] = useState<string>(
    seed?.brand_part_number || ''
  );
  const [vendor, setVendor] = useState<string>(seed?.vendor || '');
  const [partCost, setPartCost] = useState<string>(
    seed?.part_cost != null ? String(seed.part_cost) : ''
  );
  const [laborCost, setLaborCost] = useState<string>(
    seed?.labor_cost != null ? String(seed.labor_cost) : ''
  );
  const [isInstalled, setIsInstalled] = useState<boolean>(
    seed ? seed.is_installed === 1 : true
  );
  const [notes, setNotes] = useState<string>(seed?.notes || '');
  const [photoUrl, setPhotoUrl] = useState<string>(seed?.photo_url || '');
  ...
  useEffect(() => {
    ...
    void api
      ...
      .then((rows) => setOpenPlans(rows.filter((p) => p.plan_kind === 'upgrade')))
      .catch(() => setOpenPlans([]));
  ...
  useEffect(() => {
    ...
    const sessionKey =
      upgradeToEdit?.id || appliedPlanId || (defaults ? `d:${defaults.title}` : 'new');
    if (sessionRef.current === sessionKey) return;
    sessionRef.current = sessionKey;

    if (isEditing && upgradeToEdit) {
      setDate(upgradeToEdit.date);
      setOdometer(upgradeToEdit.odometer != null ? String(upgradeToEdit.odometer) : '');
      setTitle(upgradeToEdit.title);
      setCategory(upgradeToEdit.category || 'Performance');
      setBrandPartNumber(upgradeToEdit.brand_part_number || '');
      setVendor(upgradeToEdit.vendor || '');
      setPartCost(String(upgradeToEdit.part_cost));
      setLaborCost(String(upgradeToEdit.labor_cost));
      setIsInstalled(upgradeToEdit.is_installed === 1);
      setNotes(upgradeToEdit.notes || '');
      setPhotoUrl(upgradeToEdit.photo_url || '');
    } else {
      setDate(defaults?.date || new Date().toISOString().split('T')[0]);
      setOdometer(defaults?.odometer != null ? String(defaults.odometer) : '');
      setTitle(defaults?.title || '');
      setCategory(defaults?.category || 'Performance');
      setBrandPartNumber(defaults?.brand_part_number || '');
      setVendor(defaults?.vendor || '');
      setPartCost(defaults?.part_cost != null ? String(defaults.part_cost) : '');
      setLaborCost(defaults?.labor_cost != null ? String(defaults.labor_cost) : '');
      setIsInstalled(defaults?.is_installed !== 0);
      setNotes(defaults?.notes || '');
      setPhotoUrl(defaults?.photo_url || '');
    }
  }, [isOpen, upgradeToEdit?.id, defaults?.title, appliedPlanId]);
      ...
      if (upgradeToEdit) {
        ...
        if (localAppliedPlanId) {
          try {
            ...
            console.warn('Mod saved but plan could not be marked applied', planErr);
          }
        }
      }
                ...
                {openPlans.map((p) => (
                  <option key={p.id} value={p.id}>
                    ...
                    {p.estimated_total != null
                      ? ` ($${asNumber(p.estimated_total).toFixed(2)})`
                      : ''}
                  </option>
                ))}
              </select>
