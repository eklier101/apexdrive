...
import { ServiceItem, InventoryItem, ServicePartDraft } from '../../types';
...
export const ServiceModal: React.FC<ServiceModalProps> = ({
  ...
  const [date, setDate] = useState<string>(
    serviceToEdit?.date || new Date().toISOString().split('T')[0]
  );
  const [odometer, setOdometer] = useState<string>(
    serviceToEdit ? String(serviceToEdit.odometer) : ''
  );
  const [serviceType, setServiceType] = useState<string>(
    serviceToEdit?.service_type || 'Oil Change'
  );
  const [title, setTitle] = useState<string>(
    serviceToEdit?.title || 'Oil & Filter Change'
  );
  const [description, setDescription] = useState<string>(
    serviceToEdit?.description || ''
  );
  const [partsCost, setPartsCost] = useState<string>(
    serviceToEdit ? String(serviceToEdit.parts_cost) : ''
  );
  const [laborCost, setLaborCost] = useState<string>(
    serviceToEdit ? String(serviceToEdit.labor_cost) : ''
  );
  const [isDiy, setIsDiy] = useState<boolean>(
    serviceToEdit ? serviceToEdit.is_diy === 1 : false
  );
  const [serviceProvider, setServiceProvider] = useState<string>(
    serviceToEdit?.service_provider || ''
  );
  const [receiptImage, setReceiptImage] = useState<string>(
    serviceToEdit?.receipt_image || ''
  );
  ...
  useEffect(() => {
    ...
    const sessionKey = serviceToEdit?.id ?? 'new';
  ...
  }, [isOpen, serviceToEdit?.id]);

  // If inventory loads after the modal opens, default new services to inventory mode once.
  useEffect(() => {
    if (!isOpen || serviceToEdit || costModeTouchedRef.current) return;
    if (stockCount(inventoryItems) > 0) {
      setCostMode('inventory');
    }
  }, [isOpen, serviceToEdit, inventoryItems]);
  ...
  const handleTypeChange = (type: string) => {
    setServiceType(type);
    if (!serviceToEdit && (!title || SERVICE_TYPES.includes(title))) {
      setTitle(type);
    }
  };
          ...
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-lg font-bold text-white">
                {serviceToEdit ? 'Edit Service Record' : 'Log Maintenance & Service'}
              </h2>
            ...
            <div className="space-y-3 bg-slate-800/40 border border-slate-700/60 rounded-2xl p-4">
              ...
              {inventoryItems.length === 0 ? (
                <p className="text-xs text-amber-400/80">
                  No inventory yet — add parts in the Parts Inventory section above, then come back here.
                </p>
