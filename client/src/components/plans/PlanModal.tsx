...
export const PlanModal: React.FC<PlanModalProps> = ({
  ...
  const addOwnedFromInventory = (item: InventoryItem) => {
    ...
    setParts((prev) => {
      if (prev.some((p) => p.inventory_item_id === item.id)) return prev;
      return [
        ...prev,
        {
          key: newKey(),
  ...
  const markPartBought = (key: string, inventoryItemId?: string) => {
    if (inventoryItemId) {
    ...
    updatePart(key, { acquisition: 'owned' });
  };
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <Plus className="w-3.5 h-3.5" /> To buy
                </button>
              </div>
            </div>

            <div className="rounded-2xl border border-amber-500/20 bg-amber-500/5 p-3 space-y-2">
              <div className="text-[10px] uppercase font-semibold text-amber-300/90 flex items-center gap-1">
                <Package className="w-3 h-3" /> Already in inventory — tap to add as bought
              </div>
              {inStock.length === 0 ? (
                <p className="text-[11px] text-slate-500">
                  No stock yet. Add parts under Parts, or use “To buy” for items you still need.
                </p>
              ) : (
                <div className="flex flex-wrap gap-1.5 max-h-28 overflow-y-auto">
                  {inStock.map((item) => {
                    const selected = parts.some((p) => p.inventory_item_id === item.id);
                    return (
                      <button
                        key={item.id}
                        type="button"
                        onClick={() =>
                          selected
                            ? setParts((prev) => prev.filter((p) => p.inventory_item_id !== item.id))
                            : addOwnedFromInventory(item)
                        }
                        className={`px-2.5 py-1.5 rounded-lg text-[11px] font-medium border transition ${
                          selected
                            ...
                            : 'bg-slate-900/60 text-amber-300 border-amber-500/20 hover:bg-amber-500/15'
                        }`}
                      >
                        {item.name}
            ...
            {parts.length === 0 ? (
              <p className="text-[11px] text-slate-500 text-center py-2">
                Optional — add stock you already have, or parts you still need to buy.
              </p>
            ) : (
              <div className="space-y-2">
                {parts.map((p) => (
                  ...
                  >
                    ...
                    {p.acquisition === 'owned' && inStock.length > 0 && (
                      <select
                        value={p.inventory_item_id || ''}
                        onChange={(e) => {
                          const id = e.target.value;
                          if (!id) {
                            updatePart(p.key, { inventory_item_id: null });
                            return;
                          }
                          markPartBought(p.key, id);
                        }}
                        className="w-full px-2.5 py-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-white"
                      >
                        <option value="">Bought — not linked to inventory</option>
                        {inStock.map((item) => (
                    ...
                    {p.acquisition !== 'owned' && inStock.length > 0 && (
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          markPartBought(p.key, e.target.value);
                          e.target.value = '';
                        }}
                        className="w-full px-2.5 py-2 bg-slate-900 border border-slate-700 rounded-lg text-xs text-slate-300"
                      >
                        <option value="">Mark bought &amp; link inventory…</option>
                        {inStock.map((item) => (
                          <option key={item.id} value={item.id}>
                            {item.name}
                            {item.package_size ? ` (${item.package_size})` : ''}
