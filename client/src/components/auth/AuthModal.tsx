...
import { applyServerBootstrap } from '../../api/serverBootstrap';
...
export const AuthModal: React.FC = () => {
  ...
  const [firstUserIsAdmin, setFirstUserIsAdmin] = useState(false);

  useEffect(() => {
    if (!isNative) return;
    void applyServerBootstrap().then((url) => {
  ...
  useEffect(() => {
    ...
    const check = async () => {
      try {
        // Native: only probe when a server URL is available (uses current getServerUrl).
        if (isNative && !normalizeUrl(serverUrl) && !getServerUrl()) {
          if (!cancelled) setFirstUserIsAdmin(false);
          return;
        }
        const status = await api.authStatus();
        if (!cancelled) setFirstUserIsAdmin(!status.hasUsers);
      } catch {
        if (!cancelled) setFirstUserIsAdmin(false);
      }
    };
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-4 py-12 app-header-safe relative overflow-hidden">
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-800 rounded-3xl p-6 sm:p-8 shadow-2xl shadow-black/50">
          ...
          {isRegister && firstUserIsAdmin && (
            <div className="mb-5 p-3.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-200 text-xs font-medium leading-relaxed flex items-start gap-2">
              <ShieldCheck className="w-4 h-4 text-amber-400 shrink-0 mt-0.5" />
              <span>You’ll be the admin — first account on this server can manage users.</span>
            </div>
          )}
