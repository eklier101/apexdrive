...
import { syncReminderNotifications } from '../api/reminderNotifications';
...
export const RemindersPage: React.FC = () => {
  ...
  const loadReminders = async () => {
    ...
    try {
      ...
      setReminders(data);
      const label = activeVehicle.name || `${activeVehicle.year} ${activeVehicle.make}`;
      void syncReminderNotifications(data, label);
    } catch (err) {
      console.error(err);
    } finally {
      setLoading(false);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Set miles and/or months (0 = unused). Logging a matching service auto-resets the interval.
            Notifications fire when due soon or overdue.
          </p>
        </div>
        <button
          onClick={() => {
