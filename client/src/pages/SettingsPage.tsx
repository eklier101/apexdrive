...
export const SettingsPage: React.FC = () => {
  ...
  const handleSaveServerUrl = async () => {
    ...
    try {
      ...
      setConnectionStatus('Connection failed. Verify IP, port, and Wi-Fi network.');
    } finally {
    ...
    <div className="space-y-6 pb-24 md:pb-12 max-w-4xl mx-auto">
      ...
      <div>
        ...
        <p className="text-xs text-slate-400 mt-0.5">
          Backend server connection, APK auto-updates, vehicle management, and backups
        </p>
      </div>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-white">In-App APK Updater & Downloads</h2>
              <p className="text-xs text-slate-400">
                Self-hosted Android updates with Wi-Fi auto-download
              </p>
            </div>
          </div>
          ...
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 text-xs">
            <div>
              ...
              {latestRelease?.release_notes && (
                <div className="text-slate-400 mt-1">
                  Release Notes: {latestRelease.release_notes}
                </div>
              )}
              {lastChecked && (
            ...
            <div className="flex items-center gap-2">
              ...
              {hasUpdate && !downloadedUri && (
                ...
                >
                  <Download className="w-3.5 h-3.5" />
                  <span>{isDownloading ? `Downloading (${downloadProgress}%)` : 'Download APK'}</span>
                </button>
              )}
        ...
        {/* Auto-Download over Wi-Fi Setting Toggle */}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-slate-400">
                Silently download new APK versions in background when connected to Wi-Fi
              </div>
            </div>
          </div>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center gap-2.5 mb-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Host address of your self-hosted ApexDrive server (Docker, VM, or homelab)
            </p>
          </div>
        </div>
      ...
      <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-400">
                Encrypted remote HTTPS domain routing for ApexDrive
              </p>
            </div>
          </div>
        ...
        <div className="p-4 bg-slate-950/40 rounded-2xl border border-slate-800/80 space-y-2 text-xs text-slate-400 leading-relaxed">
          ...
          <p>
            Point your <code className="text-brand-400 bg-slate-800/80 px-1.5 py-0.5 rounded font-mono">cloudflared</code> ingress rule to your server (e.g. <code className="text-emerald-400 bg-slate-800/80 px-1.5 py-0.5 rounded font-mono">http://localhost:8090</code> or your LAN IP).
          </p>
          <p>
            When you open your custom domain (e.g. <code className="text-brand-400 bg-slate-800/80 px-1.5 py-0.5 rounded font-mono">https://garage.yourdomain.com</code>), the web app and Android APK will automatically utilize HTTPS and Cloudflare's edge security.
          </p>
        </div>
      </div>
