...
import { allowsSideloadUpdates } from '../utils/distribution';

export const SettingsPage: React.FC = () => {
  const isNative = Capacitor.isNativePlatform();
  const sideloadUpdates = allowsSideloadUpdates();
  ...
  return (
    <div className="space-y-6 pb-24 md:pb-12 max-w-4xl mx-auto">
        ...
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              {apkAvailable && latestRelease?.apk_valid !== false ? (
                <a
                ...
                <span className="text-xs text-amber-400/90 font-medium">
                  {latestRelease?.apk_valid === false
                    ? 'APK on server failed integrity check'
      ...
      {/* Sideload APK only: full updater + Wi-Fi autodownload (hidden on Play builds) */}
      {isNative && sideloadUpdates && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
            ...
            <div className="flex items-center gap-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-500 mt-1">
                  Updates are checked for a real signed APK before install. If Install still says
                  “App not installed”, uninstall the old app once (signing key changed), then install again.
                </p>
                {latestRelease?.apk_valid === false && (
                  <p className="text-[11px] text-rose-400 mt-1.5">
                    Server APK failed integrity check
                    {latestRelease.apk_error ? `: ${latestRelease.apk_error}` : ''}. Download is disabled until a valid package is uploaded.
                  </p>
                )}
              </div>
            </div>
      ...
      {isNative && !sideloadUpdates && (
        <div className="bg-slate-900/90 border border-slate-800 rounded-3xl p-6 shadow-sm">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <h2 className="text-base font-bold text-white">App updates</h2>
              <p className="text-xs text-slate-400">
                This Google Play build updates through the Play Store (v{currentVersion}).
                In-app APK installs are disabled.
              </p>
            </div>
          </div>
        </div>
