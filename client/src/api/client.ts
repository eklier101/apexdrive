...
export const api = {
  ...
  async authStatus(): Promise<{ hasUsers: boolean; count: number }> {
    const res = await fetchWithBase('/api/auth/status');
    if (!res.ok) throw new Error('Failed to fetch auth status');
    return res.json();
  },

  async adminListUsers(): Promise<{ users: AdminUser[] }> {
    const res = await fetchWithBase('/api/admin/users');
    if (!res.ok) {
  ...
  async adminSetRole(id: string, role: string): Promise<{ user: User }> {
    const res = await fetchWithBase(`/api/admin/users/${id}/role`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  async adminResetPassword(id: string, password: string): Promise<void> {
    const res = await fetchWithBase(`/api/admin/users/${id}/reset-password`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async adminDeleteUser(id: string): Promise<void> {
    const res = await fetchWithBase(`/api/admin/users/${id}`, { method: 'DELETE' });
    if (!res.ok) {
